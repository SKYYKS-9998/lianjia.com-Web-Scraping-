...
shanghai_mean = shanghai_df['rent'].mean()
shanghai_mean = round(shanghai_mean, 2)
shanghai_high = shanghai_df['rent'].max()
shanghai_low = shanghai_df['rent'].min()
shanghai_mid = shanghai_df['rent'].median()
shanghai_mid = round(shanghai_mid, 2)

guangzhou_mean = guangzhou_df['rent'].mean()
guangzhou_mean = round(guangzhou_mean, 2)
guangzhou_high = guangzhou_df['rent'].max()
guangzhou_low = guangzhou_df['rent'].min()
guangzhou_mid = guangzhou_df['rent'].median()
guangzhou_mid = round(guangzhou_mid, 2)

shenzhen_mean = shenzhen_df['rent'].mean()
shenzhen_mean = round(shenzhen_mean, 2)
shenzhen_high = shenzhen_df['rent'].max()
shenzhen_low = shenzhen_df['rent'].min()
shenzhen_mid = shenzhen_df['rent'].median()
shenzhen_mid = round(shenzhen_mid, 2)

quanzhou_mean = quanzhou_df['rent'].mean()
quanzhou_mean = round(quanzhou_mean, 2)
quanzhou_high = quanzhou_df['rent'].max()
quanzhou_low = quanzhou_df['rent'].min()
quanzhou_mid = quanzhou_df['rent'].median()
...
plt.xlabel('城市')
plt.ylabel('租金均价(元)')
plt.savefig('fig_q2\\五个城市租金均价.png')
...
plt.xlabel('城市')
plt.ylabel('租金中位数(元)')
plt.savefig('fig_q2\\五个城市租金中位数.png')
...
plt.xlabel('城市')
plt.ylabel('租金最高价(元)')
plt.savefig('fig_q2\\五个城市租金最高价.png')
